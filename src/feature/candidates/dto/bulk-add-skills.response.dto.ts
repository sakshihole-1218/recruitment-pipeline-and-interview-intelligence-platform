...
export class BulkAddSkillsCandidateResultDto {
  ...
  @ApiProperty({ type: [String], description: 'Skill UUIDs newly created for the candidate' })
  added_skill_ids: string[];

  @ApiProperty({ type: [String], description: 'Skill UUIDs reactivated from soft-deleted mappings' })
  reactivated_skill_ids: string[];

  @ApiProperty({ type: [String], description: 'Skill UUIDs already present and therefore skipped' })
...
export class BulkAddSkillsResponseDto {
  @ApiProperty({
    type: [String],
    description: 'Skill UUIDs that were invalid (format or not found) and skipped for all candidates',
  })
