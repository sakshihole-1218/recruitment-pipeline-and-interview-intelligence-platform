...
@Injectable()
export class BulkAssignRecruiterUseCase {
  constructor(
  ...
  async execute(dto: BulkAssignRecruiterDto, actorUserId?: string): Promise<BulkOperationResultResponseDto> {
    ...
    const recruiterExists = await this.dataSource.transaction((manager) =>
      this.referenceRepository.userExists(dto.recruiter_user_id, manager),
    );
    ...
    for (const applicationId of dto.application_ids) {
      try {
        await this.dataSource.transaction(async (manager) => {
          ...
          const now = new Date();
          const oldRecruiterId = app.assigned_recruiter_user_id;

          app.assigned_recruiter_user_id = dto.recruiter_user_id;
          ...
          await this.activityWriter.log(
            ActivityLogBuilder.build({
              ...
              actorUserId,
              oldValues: { assigned_recruiter_user_id: oldRecruiterId },
              newValues: { assigned_recruiter_user_id: dto.recruiter_user_id },
              actionAt: now,
              ipAddress: null,
              userAgent: null,
            }),
